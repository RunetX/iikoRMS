...
Процедура ИмяФайлаДанныхНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	#Если ВебКлиент тогда 
		Сообщить("В Веб клиенте расширение работы с файлами недоступно! Используйте обработку в режиме Тонкого клиента!", СтатусСообщения.Важное);
		Возврат;			
	#КонецЕсли
	...
	ДиалогВыбораКаталогЭкспорта.Показать(Новый ОписаниеОповещения("ИмяФайлаДанныхНачалоВыбораЗавершение", ЭтаФорма, Новый Структура("ДиалогВыбораКаталогЭкспорта", ДиалогВыбораКаталогЭкспорта)));
...
&НаКлиенте
Процедура ПоместитьФайлВоВременноеХранилищеЗавершение(ФайлВыбран, Адрес, ПолноеИмяВыбранногоФайла, ПараметрыОповещения) Экспорт
	
    ИмяФайлаДанных = ОбщегоНазначенияКлиентСервер.РазложитьПолноеИмяФайла(ПолноеИмяВыбранногоФайла).Имя;
    
	Если ИмяФайлаДанных = "Приходная накладная.csv"	Тогда
		АдресПН = Адрес;
	ИначеЕсли ИмяФайлаДанных = "Акт реализации.csv" 	Тогда
		АдресАР = Адрес;
	ИначеЕсли ИмяФайлаДанных = "Внутреннее перемещение.csv" 	Тогда
		АдресВП = Адрес;
	ИначеЕсли ИмяФайлаДанных = "Акт приготовления.csv" 	Тогда
		АдресАП = Адрес;	
	ИначеЕсли ИмяФайлаДанных = "Акт переработки.csv" 	Тогда
		АдресАПЕР = Адрес;
	ИначеЕсли ИмяФайлаДанных = "Акт списания.csv"	Тогда
		АдресАС = Адрес;		
	ИначеЕсли ИмяФайлаДанных = "Расходная накладная.csv" Тогда
		АдресРН = Адрес;
	ИначеЕсли ИмяФайлаДанных = "Прием кассовой выручки.csv" Тогда
		АдресАРиБезнал = Адрес;
	ИначеЕсли ИмяФайлаДанных = "Возвратная накладная.csv" Тогда
		АдресВН = Адрес;		
	ИначеЕсли ИмяФайлаДанных = "Инвентаризация.csv" Тогда
		АдресИН = Адрес;
	ИначеЕсли ИмяФайлаДанных = "Акты оказания услуг.csv" Тогда
		АдресАУ = Адрес;		
	ИначеЕсли ИмяФайлаДанных = "Акты приёма услуг.csv" Тогда
		АдресАПУ = Адрес;
	ИначеЕсли ИмяФайлаДанных = "Акт разбора.csv" Тогда
		АдресАРаз = Адрес;			
	КонецЕсли;
...
Процедура КонтрагентыСоздать(Команда)
	...
	ПоказатьВопрос(Новый ОписаниеОповещения("КонтрагентыСоздатьЗавершение", ЭтаФорма, Новый Структура("ТекДанные", ТекДанные)), "Создать контрагента с названием: " + КодНаименованияАйко  +  " и кодом: " + код + "?", РежимДиалогаВопрос.ДаНет);
КонецПроцедуры

&НаКлиенте
Процедура КонтрагентыСоздатьЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
...
Процедура КонтрагентыСоздатьВсех(Команда)
	...
	ПоказатьВопрос(Новый ОписаниеОповещения("КонтрагентыСоздатьВсехЗавершение", ЭтаФорма), "Создать всех контрагентов?",РежимДиалогаВопрос.ДаНет,0);
	
КонецПроцедуры

&НаКлиенте
Процедура КонтрагентыСоздатьВсехЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
...
Процедура КонтрагентыЗаполнить(Команда)
	...
	ПоказатьВопрос(Новый ОписаниеОповещения("КонтрагентыЗаполнитьЗавершение", ЭтаФорма), "Заполнить соответствие по ИНН/названию?",РежимДиалогаВопрос.ДаНет,0);

КонецПроцедуры

&НаКлиенте
Процедура КонтрагентыЗаполнитьЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
...
Процедура КонтрагентыОчистить(Команда)
	ПоказатьВопрос(Новый ОписаниеОповещения("КонтрагентыОчиститьЗавершение", ЭтаФорма), "Очистить таблицу соответствий?",РежимДиалогаВопрос.ДаНет,0);
КонецПроцедуры

&НаКлиенте
Процедура КонтрагентыОчиститьЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
...
Процедура НоменклатураСоздать(Команда)
	...
	ПоказатьВопрос(Новый ОписаниеОповещения("НоменклатураСоздатьЗавершение", ЭтаФорма, Новый Структура("ТекДанные", ТекДанные)), "Создать элемент номенклатуры с названием: " + ТекДанные.ННаименованиеИИКО + 
	" и кодом: " + Код + "?", РежимДиалогаВопрос.ДаНет);	
	
КонецПроцедуры

&НаКлиенте
Процедура НоменклатураСоздатьЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
...
Процедура НоменклатураСоздатьВсе(Команда)
	...
	ПоказатьВопрос(Новый ОписаниеОповещения("НоменклатураСоздатьВсеЗавершение", ЭтаФорма), "Создать все номенклатурные позиции?",РежимДиалогаВопрос.ДаНет,0);
	
КонецПроцедуры

&НаКлиенте
Процедура НоменклатураСоздатьВсеЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
...
Процедура ЗаполнитьНоменклатура(Команда)
	...
	ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполнитьНоменклатураЗавершение", ЭтаФорма), "Заполнить соответствия по названию/коду?",РежимДиалогаВопрос.ДаНет,0);
КонецПроцедуры

&НаКлиенте
Процедура ЗаполнитьНоменклатураЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
...
Процедура ОчиститьНоменклатура(Команда)
	...
	ПоказатьВопрос(Новый ОписаниеОповещения("ОчиститьНоменклатураЗавершение", ЭтаФорма), "Очистить таблицу соответствий?",РежимДиалогаВопрос.ДаНет,0);
	
КонецПроцедуры

&НаКлиенте
Процедура ОчиститьНоменклатураЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
...
Функция ПолучитьСтавкуНДС(СтавкаЧисло,Сумма,СуммаНДС)
	...
	Если Укр = 0 Тогда
		...
		ИначеЕсли СтавкаЧисло = 10 Тогда
...
Процедура ЗаполнитьДокументы(тззз,ВидДокумента,ДатаПрефикс)
	...
	Для Каждого Стр из таб Цикл
		...
		ИначеЕсли ВидДокумента = "Акт оказания услуг" или ВидДокумента = "Акт приёма услуг" тогда
			 ...
			 Стрр.Документ = СокрЛП(Стр.НомерДок) + "!" + "АктОказанияУслуг";
			 Продолжить;
 		ИначеЕсли ВидДокумента <>  "Прием кассовой выручки"  Тогда
			Стрр.Организация = СокрЛП(Стр.ЮЛнаименование);
			Стрр.СкладКодАйко = Стр.СкладКод;
...
Процедура ЗагрузитьДокументыИзФайлов()
	...
	Если ПН  тогда
		...
		Если АдресПН = "" Или АдресПН = Неопределено Тогда    
			ОшибкаФайла = Истина;
		КонецЕсли;
		
		Если ОшибкаФайла = Ложь тогда
			ВременныйФайл = ПолучитьИмяВременногоФайла();
			ДвоичныеДанныеПН = ПолучитьИзВременногоХранилища(АдресПН);
			Если ДвоичныеДанныеПН = Неопределено Тогда
				Сообщить("Не удалось прочитать файл приходных накладных " + Каталог1с + "\Приходная накладная.csv", СтатусСообщения.Важное);
				АдресПН = Неопределено;
			Иначе
				ДвоичныеДанныеПН.Записать(ВременныйФайл);		
				Текст = Новый ЧтениеТекста();
				Текст.Открыть(ВременныйФайл,КодировкаТекста.ANSI);
				АдресПН = "";
	...
	Если АР  тогда
		...
		Если ОшибкаФайла = Ложь тогда 
			ВременныйФайл = ПолучитьИмяВременногоФайла();
			ДвоичныеДанныеАР = ПолучитьИзВременногоХранилища(АдресАР);
			Если ДвоичныеДанныеАР = Неопределено Тогда
				Сообщить("Не удалось прочитать файл актов реализации " + Каталог1с + "\Акт реализации.csv", СтатусСообщения.Важное);
				АдресАР = неопределено;
			Иначе
				ДвоичныеДанныеАР.Записать(ВременныйФайл);		
				Текст1 = Новый ЧтениеТекста();
				Текст1.Открыть(ВременныйФайл,КодировкаТекста.ANSI);
	...
	Если ВП  тогда		
		...
		Если ОшибкаФайла = Ложь тогда
			...
			ВременныйФайл = ПолучитьИмяВременногоФайла();
			ДвоичныеДанныеВП = ПолучитьИзВременногоХранилища(АдресВП);
			Если ДвоичныеДанныеВП = Неопределено Тогда
				Сообщить("Не удалось прочитать файл внутренних перемещений " + Каталог1с + "\Внутреннее перемещение.csv", СтатусСообщения.Важное);
				АдресВП = неопределено;
			Иначе
				ДвоичныеДанныеВП.Записать(ВременныйФайл);		
				Текст2 = Новый ЧтениеТекста();
				Текст2.Открыть(ВременныйФайл,КодировкаТекста.ANSI);
	...
	Если АП  тогда
		...
		Если ОшибкаФайла = Ложь тогда
			ВременныйФайл = ПолучитьИмяВременногоФайла();
			ДвоичныеДанныеАП = ПолучитьИзВременногоХранилища(АдресАП);
			Если ДвоичныеДанныеАП = Неопределено Тогда
				Сообщить("Не удалось прочитать файл актов приготовления " + Каталог1с + "\Акт приготовления.csv", СтатусСообщения.Важное);
				АдресАп = неопределено;
			Иначе
				ДвоичныеДанныеАП.Записать(ВременныйФайл);		
				Текст3 = Новый ЧтениеТекста();
				Текст3.Открыть(ВременныйФайл,КодировкаТекста.ANSI);
	...
	Если АПЕР  тогда
		...
		Если ОшибкаФайла = Ложь тогда 
			...
			ВременныйФайл = ПолучитьИмяВременногоФайла();
			ДвоичныеДанныеАпер = ПолучитьИзВременногоХранилища(АдресАпер);
			Если ДвоичныеДанныеАпер = Неопределено Тогда
				Сообщить("Не удалось прочитать файл актов переработки " + Каталог1с + "\Акт переработки.csv", СтатусСообщения.Важное);
				АдресАпер = неопределено;
			Иначе
				ДвоичныеДанныеАпер.Записать(ВременныйФайл);		
				Текст5 = Новый ЧтениеТекста();
				Текст5.Открыть(ВременныйФайл,КодировкаТекста.ANSI);
	...
	Если АС тогда		
		...
		Если ОшибкаФайла =  Ложь тогда 
			ВременныйФайл = ПолучитьИмяВременногоФайла();
			ДвоичныеДанныеАС = ПолучитьИзВременногоХранилища(АдресАС);
			Если ДвоичныеДанныеАС = Неопределено Тогда
				Сообщить("Не удалось прочитать файл актов списания " + Каталог1с + "\Акт списания.csv", СтатусСообщения.Важное);
				АдресАС = Неопределено;
			Иначе
				ДвоичныеДанныеАС.Записать(ВременныйФайл);		
				Текст6 = Новый ЧтениеТекста();
				Текст6.Открыть(ВременныйФайл,КодировкаТекста.ANSI);
				АдресАС = ПолучитьТЗ(Текст6,НачИменаКолонок6,УмныеИменаКолонок6);
				
				Если АдресАС = Ложь Тогда
					Сообщить("Выбранный файл актов списания пуст!!!", СтатусСообщения.Важное);
					Возврат;
				КонецЕсли;
	...
	Если Рн тогда		
		...
		Если ОшибкаФайла = Ложь тогда 
			...
			ВременныйФайл = ПолучитьИмяВременногоФайла();
			ДвоичныеДанныеРН = ПолучитьИзВременногоХранилища(АдресРН);
			Если ДвоичныеДанныеРН = Неопределено Тогда
				Сообщить("Не удалось прочитать файл расходных накладных " + Каталог1с + "\Расходная накладная.csv", СтатусСообщения.Важное);
				АдресРН = неопределено;
			Иначе
				ДвоичныеДанныеРН.Записать(ВременныйФайл);		
				Текст8 = Новый ЧтениеТекста();
				Текст8.Открыть(ВременныйФайл,КодировкаТекста.ANSI);
	...
	Если АР и (БезналичныеОплаты = "ИзПриемаКассовойВыручки") тогда 		
		Если АдресАРиБезнал = "" Тогда    
			ОшибкаФайла = Истина;;
		КонецЕсли;
		
		Если ОшибкаФайла = Ложь тогда 
			ВременныйФайл = ПолучитьИмяВременногоФайла();
			ДвоичныеДанныеАРиБезнал = ПолучитьИзВременногоХранилища(АдресАРиБезнал);
			Если ДвоичныеДанныеАРиБезнал = Неопределено Тогда
				Сообщить("Не удалось прочитать файл приемов кассовой выручки " + Каталог1с + "\Прием кассовой выручки.csv", СтатусСообщения.Важное);
				АдресАРиБезнал = Неопределено;
			Иначе
				ДвоичныеДанныеАРиБезнал.Записать(ВременныйФайл);		
				Текст4 = Новый ЧтениеТекста();
				Текст4.Открыть(ВременныйФайл,КодировкаТекста.ANSI);
	...
	Если ВН  тогда				
		...
		Если ОшибкаФайла = Ложь тогда 
			...
			ВременныйФайл = ПолучитьИмяВременногоФайла();
			ДвоичныеДанныеВН = ПолучитьИзВременногоХранилища(АдресВН);
			Если ДвоичныеДанныеВН = Неопределено Тогда
				Сообщить("Не удалось прочитать файл возвратных накладных " + Каталог1с + "\Возвратная накладная.csv", СтатусСообщения.Важное);
				АдресВН =  неопределено;
			Иначе
				ДвоичныеДанныеВН.Записать(ВременныйФайл);		
				Текст9 = Новый ЧтениеТекста();
				Текст9.Открыть(ВременныйФайл,КодировкаТекста.ANSI);
	...
	Если ИН тогда 		
		...
		Если ОшибкаФайла = Ложь тогда 
		    ВременныйФайл = ПолучитьИмяВременногоФайла();
			ДвоичныеДанныеИН = ПолучитьИзВременногоХранилища(АдресИН);
			Если ДвоичныеДанныеИН = Неопределено Тогда
				Сообщить("Не удалось прочитать файл инвентаризации " + Каталог1с + "\Инвентаризация.csv", СтатусСообщения.Важное);
				АдресИН  = неопределено;
			Иначе
				ДвоичныеДанныеИН.Записать(ВременныйФайл);		
				Текст7 = Новый ЧтениеТекста();
				Текст7.Открыть(ВременныйФайл,КодировкаТекста.ANSI);
	...
	Если АУ тогда 		 
		 ...
		 Если не ОшибкаФайла тогда 
			...
			ВременныйФайл = ПолучитьИмяВременногоФайла();
			ДвоичныеДанныеАУ = ПолучитьИзВременногоХранилища(АдресАУ);
			Если ДвоичныеДанныеАУ = Неопределено Тогда
				Сообщить("Не удалось прочитать файл актов оказания услуг " + Каталог1с + "\Акты оказания услуг.csv", СтатусСообщения.Важное);
				АдресАУ = Неопределено;
			Иначе
				ДвоичныеДанныеАУ.Записать(ВременныйФайл);		
				Текст10  = Новый ЧтениеТекста();
				Текст10.Открыть(ВременныйФайл,КодировкаТекста.ANSI);
	...
	Если АПУ тогда 
		...
		Если не ОшибкаФайла тогда 
			ВременныйФайл = ПолучитьИмяВременногоФайла();
			ДвоичныеДанныеАПУ = ПолучитьИзВременногоХранилища(АдресАПУ);
			Если ДвоичныеДанныеАПУ = Неопределено Тогда
				Сообщить("Не удалось прочитать файл актов приёма услуг " + Каталог1с + "\Акты приёма услуг.csv", СтатусСообщения.Важное);
				АдресАПУ = Неопределено;
			Иначе
				ДвоичныеДанныеАПУ.Записать(ВременныйФайл);		
				Текст11  = Новый ЧтениеТекста();
				Текст11.Открыть(ВременныйФайл,КодировкаТекста.ANSI);
	...
	Если Араз  тогда
		...
		Если АдресАРаз = "" Тогда    
			ОшибкаФайла = Истина;
		КонецЕсли;
		
		Если ОшибкаФайла = Ложь тогда
			ВременныйФайл = ПолучитьИмяВременногоФайла();
			ДвоичныеДанныеАРаз = ПолучитьИзВременногоХранилища(АдресАРаз);
			Если ДвоичныеДанныеАРаз = Неопределено Тогда
				Сообщить("Не удалось прочитать файл актов разбора " + Каталог1с + "\Акт разбора.csv", СтатусСообщения.Важное);
				АдресАРаз = Неопределено;
			Иначе
				ДвоичныеДанныеАРаз.Записать(ВременныйФайл);		
				Текст12  = Новый ЧтениеТекста();
				Текст12.Открыть(ВременныйФайл,КодировкаТекста.ANSI);
...
Процедура ЗагрузитьНажатие(Команда)
	...
	Если (Объект.Докум.Количество() > 0) Тогда           
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗагрузитьНажатиеЗавершение5", ЭтаФорма), "Вы хотите перезаполнить таблицы соответствия?",РежимДиалогаВопрос.ДаНет);
		Возврат;
	КонецЕсли;
	
	
	ПоместитьФайлыВАдреса();
		
	ЗагрузитьНажатиеФрагмент2();
КонецПроцедуры

&НаКлиенте
Процедура ПоместитьФайлыВАдреса()
	ОписаниеОповещения = Новый ОписаниеОповещения("ПоместитьФайлВоВременноеХранилищеЗавершение", ЭтаФорма);
	
Если ПН  тогда
		Попытка
			НачатьПомещениеФайла(ОписаниеОповещения,,Каталог1с + "\Приходная накладная.csv",Ложь, ЭтаФорма.УникальныйИдентификатор);
		Исключение
	...
	Если АР  тогда
		Попытка
			НачатьПомещениеФайла(ОписаниеОповещения,,Каталог1с + "\Акт реализации.csv",Ложь, ЭтаФорма.УникальныйИдентификатор);
		Исключение
	...
	Если ВП  тогда
		Попытка
			НачатьПомещениеФайла(ОписаниеОповещения,,Каталог1с + "\Внутреннее перемещение.csv",Ложь, ЭтаФорма.УникальныйИдентификатор);
		Исключение
	...
	Если АП  тогда
		Попытка
			НачатьПомещениеФайла(ОписаниеОповещения,,Каталог1с + "\Акт приготовления.csv",Ложь, ЭтаФорма.УникальныйИдентификатор);
		Исключение
	...
	Если АПЕР тогда
		Попытка
			НачатьПомещениеФайла(ОписаниеОповещения,,Каталог1с + "\Акт переработки.csv",Ложь, ЭтаФорма.УникальныйИдентификатор);
		Исключение
	...
	Если АС тогда
		Попытка
			НачатьПомещениеФайла(ОписаниеОповещения,,Каталог1с + "\Акт списания.csv",Ложь, ЭтаФорма.УникальныйИдентификатор);
		Исключение
	...
	Если Рн тогда
		Попытка
			НачатьПомещениеФайла(ОписаниеОповещения,,Каталог1с + "\Расходная накладная.csv",Ложь, ЭтаФорма.УникальныйИдентификатор);
		Исключение
	...
	Если АР и (БезналичныеОплаты = "ИзПриемаКассовойВыручки") тогда 
		Попытка
			НачатьПомещениеФайла(ОписаниеОповещения,,Каталог1с + "\Прием кассовой выручки.csv",Ложь, ЭтаФорма.УникальныйИдентификатор);
		Исключение
	...
	Если ВН  тогда
		Попытка
			НачатьПомещениеФайла(ОписаниеОповещения,,Каталог1с + "\Возвратная накладная.csv",Ложь, ЭтаФорма.УникальныйИдентификатор);
		Исключение
	...
	Если ИН тогда 
		Попытка
			НачатьПомещениеФайла(ОписаниеОповещения,,Каталог1с + "\Инвентаризация.csv",Ложь, ЭтаФорма.УникальныйИдентификатор);
		Исключение
	...
	Если АУ тогда
		Попытка
			НачатьПомещениеФайла(ОписаниеОповещения,,Каталог1с + "\Акты оказания услуг.csv",Ложь, ЭтаФорма.УникальныйИдентификатор);
		Исключение
	...
	Если АПУ тогда
		Попытка
			НачатьПомещениеФайла(ОписаниеОповещения,,Каталог1с + "\Акты приёма услуг.csv",Ложь, ЭтаФорма.УникальныйИдентификатор);
		Исключение
	...
	Если Араз  тогда
		Попытка 
			НачатьПомещениеФайла(ОписаниеОповещения,,Каталог1с + "\Акт разбора.csv",Ложь, ЭтаФорма.УникальныйИдентификатор);
		Исключение
...
&НаКлиенте
Процедура ЗагрузитьНажатиеЗавершение5(РезультатВопроса, ДополнительныеПараметры) Экспорт
...
&НаКлиенте
Процедура ЗагрузитьНажатиеФрагмент2()
	...
	ЗагрузитьНажатиеФрагмент();

КонецПроцедуры

&НаКлиенте
Процедура ЗагрузитьНажатиеФрагмент()
	
	Перем Естьче, НужнаяКонцепция, НужноеЮрЛицо, СписокКонцепций, СписокЮрЛиц, юл, Юрлицо;
...
&НаКлиенте
Процедура ПослеВыбораЮЛ(ВыбранныйЭлемент, СписокПараметров) Экспорт
...
&НаКлиенте
Процедура ЗагрузитьНажатиеЗавершение1(ДополнительныеПараметры) Экспорт
...
&НаКлиенте
Процедура ЗагрузитьНажатиеЗавершение(ДополнительныеПараметры) Экспорт
...
Процедура ЗагрузитьНаСервере()
	...
	Если ПН и ЗначениеЗаполнено(АдресПН) тогда
		...
		|Типноменклатурыназвание,Единицаизмерениякод,Единицаизмеренияназвание,ЦенасНДС,КонцепцияНазвание,КонцепцияКод,Комментарий","Колво,СуммасНДС,СуммаНДС");   // НомерГТД,
	...
	Если АУ и ЗначениеЗаполнено(АдресАУ) тогда 
		ЗаполнитьДокументы(тз10,"Акт оказания услуг",Объект.ДеньПрефикс);	
	КОнецЕсли;
...
Функция ИсправленныйЮЛИНН(иннИзАйко)    
	...
	//  Исправляем ИНН из IIKO - убираем начальные нули
	...
	Для каждого докСтр из Доки2 цикл
		...
		Если СписДок.Количество() > 1 Тогда
			...
			Для каждого СтрокаМассива из СписДок цикл
				...
				Док.Склад = Объект.Склады.НайтиСтроки(Новый Структура ("СкладНаименование",СтрокаМассива.Склад))[0].Склад;
		...
		Иначе
			...
			Док.Склад = Объект.Склады.НайтиСтроки(Новый Структура ("СкладНаименование",СписДок[0].Склад))[0].Склад;
			Док.ПодразделениеОрганизации = Объект.Склады.НайтиСтроки(Новый Структура("Склад",Док.Склад))[0].Подразделение;
			Док.Номер = НомерДокумента;
			...
			Для каждого стрр из Табчасть Цикл
				...
				Если строкк<>неопределено тогда
					стр2.СчетУчетаНДС = строкк.СчетУчетаНДСПоПриобретеннымЦенностям;
					//  Добавить указание субконто для СчетУчетаБУ и СчетУчетаНУ
...
Функция ПодготовитьНакладные(табз)
	...
	Для Каждого Стр из табз Цикл
		...
		ххх.Склад = Стр.СкладНаименование;
		...
		НайденныйКонтрагент = Объект.Контрагенты.НайтиСтроки(Новый Структура("КкодИИКО", Стр.КкодИИКО));
...
Процедура СкладыСоздать(Команда)
	...
	ПоказатьВопрос(Новый ОписаниеОповещения("СкладыСоздатьЗавершение", ЭтаФорма, Новый Структура("ТекДанные", ТекДанные)), "Создать склад с названием: " + 
	ТекДанные.СкладНаименование + "?", РежимДиалогаВопрос.ДаНет);
	
КонецПроцедуры

&НаКлиенте
Процедура СкладыСоздатьЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
...
Процедура СкладыСоздатьВсе(Команда)
	...
	ПоказатьВопрос(Новый ОписаниеОповещения("СкладыСоздатьВсеЗавершение", ЭтаФорма), "Создать все Элементы?",РежимДиалогаВопрос.ДаНет,0);
	
КонецПроцедуры

&НаКлиенте
Процедура СкладыСоздатьВсеЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
...
Процедура СкладыЗаполнить(Команда)
	...
	ПоказатьВопрос(Новый ОписаниеОповещения("СкладыЗаполнитьЗавершение", ЭтаФорма), "Заполнить соответствие по названию?",РежимДиалогаВопрос.ДаНет,0);
КонецПроцедуры

&НаКлиенте
Процедура СкладыЗаполнитьЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
...
Процедура СоздатьРНакладные(табз,префикс,ДеньПрефикс,порядок)
	...
	Для каждого докСтр из Доки цикл
		...
		Док.ПодразделениеОрганизации = Объект.Склады.НайтиСтроки(Новый Структура("Склад",Док.Склад))[0].Подразделение;
		Док.Номер = НомерДокумента;
		//  Брать из справочника "Счета учета расчетов с контрагентами
	...
	Для каждого докСтр из Доки цикл
		...
		Для каждого стрр из Табчасть Цикл
			...
			Если Строкк <>  неопределено тогда
				...
				Иначе
					Стр2.СчетУчета = Строкк.СчетУчета;
					Стр2.НовыйСчетУчета = ПланыСчетов.Хозрасчетный.НайтиПоКоду("289");
				КонецЕсли;
			Иначе
...
Процедура СоздатьВозвратныеНакладные(табз,префикс,ДеньПрефикс,порядок)
	...
	Для каждого Стр из Доки цикл
		...
		// Брать из справочника "Счета учета расчетов с контрагентами
		Док.СчетУчетаРасчетовСКонтрагентом = ПланыСчетов.Хозрасчетный.НайтиПоКоду("60.01");
		Док.СчетУчетаРасчетовПоПретензиям = ПланыСчетов.Хозрасчетный.НайтиПоКоду("76.02");
...
Процедура СоздатьИнвентаризации(табз,префикс,порядок)
	...
	Для каждого СтрокаТЧ из Табз цикл
		СтрокаТЧ.ННаименованиеИИКО = НайтиНоменклатуру(СтрокаТЧ.НКодИИКО, СтрокаТЧ.Ставкандс, СтрокаТЧ.ГруппаучетаНазвание).Номенклатура; 	
	КонецЦикла;
	...
	Для каждого Стр из Доки цикл
		...
		ТабЧасть = Новый ТаблицаЗначений;
		Табчасть = Табз.Скопировать(НСтр);
		Табчасть.Свернуть("ННаименованиеИИКО","СебестоимостьБезНДС, ИзлишкиНедостачаКоличество, Количество");
		Для каждого стрр из ТабЧасть Цикл 
			ОкрКоличество = Окр(стрр.ИзлишкиНедостачаКоличество,3);
			Стр2 = Док.Товары.Добавить();
			Стр2.Номенклатура = стрр.ННаименованиеИИКО;
	
			Строкк = НайтиСчетаДляНоменклатуры(стрр.ННаименованиеИИКО,ТаблицаЗапроса,Док.Склад);
			
			Если Строкк <>  неопределено тогда
				счетУчета = Строкк.СчетУчета;
			Иначе
				Сообщить("Не найден счет учета для номенклатуры: " + СтрокаТЧ.ННаименованиеИИКО.Наименование, СтатусСообщения.Внимание);
			КонецЕсли;
			...
			Если не СуммовойУчет тогда 
				Если Тостатки.Количество()>0 Тогда
					...
					Если Стр2.КоличествоУчет <> 0 тогда
						стр2.Сумма = стрр.СебестоимостьБезНДС;
						стр2.Цена = ?(стр2.Количество = 0, 0,стр2.Сумма / стр2.Количество);						
					КонецЕсли;
				Иначе
					стр2.Количество = стрр.Количество;
					Стр2.Сумма = стрр.СебестоимостьБезНДС;
...
Процедура СоздатьПКВ(табз,префикс,порядок, тз1, ТЗ10)	
	...
	Если ТЗ10 <> Неопределено тогда
		...
		Для каждого Стр из ДокиАУ цикл
			...
			Стр.НомерДок = Стр.НомерДок + "!" + "АктОказанияУслуг";//нужно при разборе акта оказания услуг пробовать разделитьь номера и отделить АР от АУ
	...
	Для каждого стр из Доки цикл
		Если Найти(Стр.НомерДок,"!") = 0 тогда
			Стр.НомерДок=СокрЛП(Стр.НомерДок)+"!"+"Акт Реализации";
		КонецЕсли;
		Стр.Дата = НачалоДня(ПолучитьДату(Стр.Дата));
	КонецЦикла; 	
	...
	Если СоздатьПКО тогда
		Для каждого Стр из Объект.Докум цикл
			Если Найти(Стр.ВидДок,"Акт оказания услуг") > 0 И ЗначениеЗаполнено(Стр.Док) тогда					
				Док1 = Документы.ПриходныйКассовыйОрдер.СоздатьДокумент();
				Док1.ВидОперации = Перечисления.ВидыОперацийПКО.РозничнаяВыручка;
				Док1.Заполнить(Стр.Док);		
				Док1.ВалютаДокумента =  Константы.ВалютаРегламентированногоУчета.Получить();
				Док1.Дата = Стр.Док.Дата ; 
				Док1.Организация = Стр.Док.Организация;
				Док1.Комментарий ="Отчет о розничных продажах № " + Стр.Док.Номер + " от " +  Стр.Док.Дата+""; 
				Док1.СуммаДокумента = Стр.Док.СуммаДокумента - Стр.Док.Оплата.Итог("СуммаОплаты"); 
				Док1.СчетКасса = ПланыСчетов.Хозрасчетный.НайтиПоКоду("50.01");
				Док1.СчетУчетаРасчетовСКонтрагентом = ПланыСчетов.Хозрасчетный.НайтиПоКоду("90.01.1");
				Док1.ДокументОснование= Стр.Док; 
				Док1.Контрагент = Стр.Док.Склад;
				Док1.ПринятоОт = "Гость";
				
				Док1.РасшифровкаПлатежа.Очистить();
				Строка = Док1.РасшифровкаПлатежа.Добавить();
				Строка.СуммаПлатежа = Стр.Док.СуммаДокумента - Стр.Док.Оплата.Итог("СуммаОплаты");
				Строка.СтавкаНДС =  Перечисления.СтавкиНДС.БезНДС;
				Строка.СтатьяДвиженияДенежныхСредств = Стр.Док.СтатьяДвиженияДенежныхСредств;
...
Процедура СоздатьАвансовыйОтчет(докСтр,строки,НомерДокумента, Порядок, номернаклиико, ВидДок)
	...
	Если ВидДок = "Приходная Накладная" тогда 
		Док.Склад = Объект.Склады.НайтиСтроки(Новый Структура ("СкладНаименование",докСтр.Склад))[0].Склад;
		Док.ПодразделениеОрганизации = Объект.Склады.НайтиСтроки(Новый Структура("Склад",Док.Склад))[0].Подразделение;
	КонецЕсли;
...
Процедура ГруппировкаСоздать(Команда)   
	...
	ПоказатьВопрос(Новый ОписаниеОповещения("ГруппировкаСоздатьЗавершение", ЭтаФорма), "Создать элемент номенклатуры с названием: " + 
	Элементы.ГруппаГруппировка.ПодчиненныеЭлементы.Группировка.ТекущиеДанные.СтавкаНДС + "?", РежимДиалогаВопрос.ДаНет);
	
КонецПроцедуры

&НаКлиенте
Процедура ГруппировкаСоздатьЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
...
Процедура СкладыОчистить(Команда)
	...
	ПоказатьВопрос(Новый ОписаниеОповещения("СкладыОчиститьЗавершение", ЭтаФорма), "Очистить таблицу соответствий?",РежимДиалогаВопрос.ДаНет,0);
		
КонецПроцедуры

&НаКлиенте
Процедура СкладыОчиститьЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
...
Процедура ЗаполнитьГруппировка(Команда)
	...
	ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполнитьГруппировкаЗавершение", ЭтаФорма), "Заполнить соответствия по названию?",РежимДиалогаВопрос.ДаНет,0);
КонецПроцедуры

&НаКлиенте
Процедура ЗаполнитьГруппировкаЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
...
Процедура ГруппировкаСоздатьВсе(Команда)
	ПоказатьВопрос(Новый ОписаниеОповещения("ГруппировкаСоздатьВсеЗавершение", ЭтаФорма), "Создать все номенклатурные позиции?",РежимДиалогаВопрос.ДаНет,0);

КонецПроцедуры

&НаКлиенте
Процедура ГруппировкаСоздатьВсеЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
...
Процедура СоздатьАУ(табз,префикс,ДеньПрефикс,порядок)
	...
	Для каждого ДокСтр из Доки Цикл	
		Если не ЗначениеЗаполнено(ДокСтр.Контрагент) тогда 
			номернаклиико = СокрЛП(докСтр.Номер) + "!" + "АктОказанияУслуг";
			Если докСтр.Номер = СтарыйНомер тогда
...
Процедура СкладыПередУдалениемНаСервере()
	...
	ПоказатьВопрос(Новый ОписаниеОповещения("СкладыПередУдалениемНаСервереЗавершение", ЭтаФорма), "Удалить из загрузки все документы со складом "+
	Этаформа.ТекущийЭлемент.ТекущиеДанные.СкладНаименование+"?", 
	РежимДиалогаВопрос.ДаНет); 
КонецПроцедуры

&НаКлиенте
Процедура СкладыПередУдалениемНаСервереЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
