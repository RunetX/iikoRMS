...
Функция НайтиСчетаДляНоменклатуры(ном,ТаблицаЗапроса,Склад)
	...
	Номмм = ном.ПолучитьОбъект();
	 // Идем по иерархии вверх, ищем счета учета по папкам номенклатуры
	Пока Номмм.Уровень() > 0 Цикл
		Номмм = Номмм.Родитель;
	...
	// Если нет по папкам - ищем по виду номенклатуры
	Если Строкк = Неопределено тогда 
		Строкк =  ТаблицаЗапроса.Найти(ном.Видноменклатуры,"ВидНоменклатуры");
		Если Строкк <> Неопределено тогда	
			Возврат Строкк;
		КонецЕсли;		
	КонецЕсли;
	...
	// Если не нашли вообще ничего - берем правило "при отсутствии других правил"
...
//	СТрокаНастроек = XMLСтрока(ТекущаяНастройка);
	...
	Если ТекущаяНастройка <> Неопределено Тогда                
		...
		КонецПопытки;
		ПроверитьНастройкиНаМетаданные(ТекущаяНастройка);
	КонецЕсли;
...
Функция СоздатьНоменклатуру(Код, Наименование, Категория = Неопределено, СтавкаНДСстр = Неопределено)
	...
	Если (катег.количество() = 0)  или не (ЗначениеЗаполнено(катег[0].Группа1С))  Тогда
		Сообщить("Не заполнена группа для категории " + Категория, СтатусСообщения.ОченьВажное);
		Возврат Неопределено;
	ИначеЕсли НЕ ЗначениеЗаполнено(Катег[0].ВидНоменклатуры) тогда 
		Сообщить("Не заполнен вид номенклатуры для категории " + Категория, СтатусСообщения.ОченьВажное);
		Возврат Неопределено;
	КонецЕсли;	
	...
	КонецЕсли;
	Ном.ВидНоменклатуры = Катег[0].ВидНоменклатуры;
	...
	Для Каждого Стр из таб Цикл
		...
		ИначеЕсли ВидДокумента = "Акт оказания услуг" или ВидДокумента = "Акт приёма услуг" тогда
			...
			//ВидДокумента = "Акт Реализации";
			 Стрр.Документ = СокрЛП(Стр.НомерДок) + "!" + "АктОказанияУслуг";//СокрЛП(ВидДокумента);
			 Продолжить;
 		ИначеЕсли ВидДокумента <>  "Прием кассовой выручки"  Тогда
			Стрр.Организация = СокрЛП(Стр.ЮЛнаименование);
			Стрр.СкладКодАйко = Стр.СкладКод;
	...
	Если АР  тогда
		...
		Если ОшибкаФайла = Ложь тогда 
			...
			Иначе
				...
				Текст = Новый ЧтениеТекста();
				Текст.Открыть(ВременныйФайл,КодировкаТекста.ANSI);
				АдресАР = ПолучитьТЗ(Текст,НачИменаКолонок1,УмныеИменаКолонок1);
				Тз1 = ПолучитьИзВременногоХранилища(АдресАР);
	...
	Если ВП  тогда		
		...
		Если ОшибкаФайла = Ложь тогда
			...
			Иначе
				...
				Текст = Новый ЧтениеТекста();
				Текст.Открыть(ВременныйФайл,КодировкаТекста.ANSI);
				АдресВП = ПолучитьТЗ(Текст,НачИменаКолонок2,УмныеИменаКолонок2);
				Текст.Закрыть();
				УдалитьФайлы(ВременныйФайл);
			КонецЕсли;				
		Иначе 
	...
	Если АП  тогда
		...
		Если ОшибкаФайла = Ложь тогда
			...
			Иначе
				...
				Текст = Новый ЧтениеТекста();
				Текст.Открыть(ВременныйФайл,КодировкаТекста.ANSI);
				АдресАП = ПолучитьТЗ(Текст,НачИменаКолонок3,УмныеИменаКолонок3);
	...
	Если АПЕР  тогда
		...
		Если ОшибкаФайла = Ложь тогда 
			...
			Иначе
				...
				Текст = Новый ЧтениеТекста();
				Текст.Открыть(ВременныйФайл,КодировкаТекста.ANSI);
				АдресАПЕР = ПолучитьТЗ(Текст,НачИменаКолонок3,УмныеИменаКолонок3);		
	...
	Если АС тогда		
		...
		Если ОшибкаФайла =  Ложь тогда 
			...
			Иначе
				...
				Текст = Новый ЧтениеТекста();
				Текст.Открыть(ВременныйФайл,КодировкаТекста.ANSI);
				АдресАС = ПолучитьТЗ(Текст,НачИменаКолонок6,УмныеИменаКолонок6);
	...
	Если Рн тогда		
		...
		Если ОшибкаФайла = Ложь тогда 
			...
			Иначе
				...
				Текст = Новый ЧтениеТекста();
				Текст.Открыть(ВременныйФайл,КодировкаТекста.ANSI);
                АдресРН = ПолучитьТЗ(Текст,НачИменаКолонок8,УмныеИменаКолонок8);
				Текст.Закрыть();
				УдалитьФайлы(ВременныйФайл);
			КонецЕсли;	
		Иначе 
	...
	Если АР и (БезналичныеОплаты = "ИзПриемаКассовойВыручки") тогда 		
		...
		Если ОшибкаФайла = Ложь тогда 
			...
			Иначе
				...
				Текст = Новый ЧтениеТекста();
				Текст.Открыть(ВременныйФайл,КодировкаТекста.ANSI);
                АдресАРиБезнал = ПолучитьТЗ(Текст,НачИменаКолонок4,УмныеИменаКолонок4);
				Текст.Закрыть();
				УдалитьФайлы(ВременныйФайл);
			КонецЕсли;	
		Иначе 
	...
	Если ВН  тогда				
		...
		Если ОшибкаФайла = Ложь тогда 
			...
			Иначе
				...
				Текст = Новый ЧтениеТекста();
				Текст.Открыть(ВременныйФайл,КодировкаТекста.ANSI);
                АдресВН = ПолучитьТЗ(Текст,НачИменаКолонок9,УмныеИменаКолонок9);
				Текст.Закрыть();
				УдалитьФайлы(ВременныйФайл);
			КонецЕсли;	
		Иначе
	...
	Если ИН тогда 		
		...
		Если ОшибкаФайла = Ложь тогда 
			...
			Иначе
				...
				Текст = Новый ЧтениеТекста();
				Текст.Открыть(ВременныйФайл,КодировкаТекста.ANSI);
                АдресИН = ПолучитьТЗ(Текст,НачИменаКолонок7,УмныеИменаКолонок7);
				Текст.Закрыть();
				УдалитьФайлы(ВременныйФайл);
			КонецЕсли;		
		Иначе 
	...
	Если АУ тогда 		 
		 ...
		 Если не ОшибкаФайла тогда 
			...
			Иначе
				...
				Текст  = Новый ЧтениеТекста();
				Текст.Открыть(ВременныйФайл,КодировкаТекста.ANSI);
                АдресАУ = ПолучитьТЗ(Текст, НачИменаКолонок10, УмныеИменаКолонок10);
				Текст.Закрыть();
				УдалитьФайлы(ВременныйФайл);
			КонецЕсли;	 
		Иначе
	...
	Если АПУ тогда 
		...
		Если не ОшибкаФайла тогда 
			...
			Иначе
				...
				Текст  = Новый ЧтениеТекста();
				Текст.Открыть(ВременныйФайл,КодировкаТекста.ANSI);
				АдресАПУ = ПолучитьТЗ(Текст, НачИменаКолонок11, УмныеИменаКолонок11);;
				Текст.Закрыть();
				УдалитьФайлы(ВременныйФайл);
			КонецЕсли;	 
		Иначе
	...
	Если Араз  тогда
		...
		Если ОшибкаФайла = Ложь тогда
			...
			Иначе
				...
				Текст  = Новый ЧтениеТекста();
				Текст.Открыть(ВременныйФайл,КодировкаТекста.ANSI);
				АдресАРаз = ПолучитьТЗ(Текст,НачИменаКолонок12,УмныеИменаКолонок12);
				Текст.Закрыть();
				УдалитьФайлы(ВременныйФайл);
			КонецЕсли;	 
		Иначе 
...
Процедура ЗагрузитьНажатие(Команда)
	...
	Если Элементы.Откуда.ТекущаяСтраница = Элементы.Откуда.ПодчиненныеЭлементы.Сервер Тогда
		...
		Если (Объект.Докум.Количество() > 0) Тогда           
			ПоказатьВопрос(Новый ОписаниеОповещения("ЗагрузитьНажатиеЗавершениеСоответствияССервера", ЭтаФорма), "Вы хотите перезаполнить таблицы соответствия?",РежимДиалогаВопрос.ДаНет);
			Возврат;
		КонецЕсли;
	...
	Если (Объект.Докум.Количество() > 0) Тогда           
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗагрузитьНажатиеЗавершениеСоответствия", ЭтаФорма), "Вы хотите перезаполнить таблицы соответствия?",РежимДиалогаВопрос.ДаНет);
		Возврат;
	КонецЕсли;
...
&НаКлиенте
Процедура ЗагрузитьНажатиеЗавершениеСоответствия(РезультатВопроса, ДополнительныеПараметры) Экспорт
...
&НаКлиенте
Процедура ЗагрузитьНажатиеЗавершениеСоответствияССервера(РезультатВопроса, ДополнительныеПараметры) Экспорт
...
Процедура ПоместитьФайлыВАдреса()
	ОписаниеОповещения = Новый ОписаниеОповещения("ПоместитьФайлВоВременноеХранилище", ЭтаФорма);
...
&НаКлиенте
Процедура ПоместитьФайлВоВременноеХранилище(ФайлВыбран, Адрес, ПолноеИмяВыбранногоФайла, ПараметрыОповещения) Экспорт
...
Процедура ЗагрузитьНаСервере()
	...
	Для Каждого Элемент из Объект.БухгалтерскаяКатегория Цикл
		...
		Если ЗначениеЗаполнено(строка) Тогда
			...
			Если ТекущаяНастройка.СохраненнаяНастройка.Категории.Колонки.Найти("ВидНоменклатуры") <> Неопределено тогда
				Элемент.ВидНоменклатуры = Строка.ВидНоменклатуры;					
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;    
	...
	Для каждого докСтр из Доки2 цикл
		...
		Если СписДок.Количество() > 1 Тогда
			...
			Для каждого СтрокаМассива из СписДок цикл
				...
				Док.Товары.Очистить();
				Док.Услуги.Очистить();
				Отб = Новый Структура("Номер, Склад, Контрагент",СтрокаМассива.Номер, СтрокаМассива.Склад, СтрокаМассива.Контрагент);
...
Функция ПодготовитьНакладные(табз)
	...
	Для Каждого Стр из табз Цикл
		...
		ххх.Склад = Стр.СкладНаименование;//Объект.Склады.НайтиСтроки(Новый Структура ("СкладНаименование",Стр.СкладНаименование))[0].Склад;
...
Функция ПодготовитьРНакладные(табз)
	...
	Для Каждого Стр из табз Цикл
		...
		Дата = Стр.Дата;
		ххх.Дата = ПолучитьДату(Дата);
		ххх.Номенклатура = НайтиНоменклатуру(Стр.НКодИИКО, Стр.СтавкаНДСспродаж, Стр.ГруппаучетаНазвание).Номенклатура;
...
Процедура СоздатьАктыСписания(строки,префикс,порядок)
	...
	Для каждого докСтр из Доки цикл
		...
		ИначеЕСли КолСубконто=2 Тогда
			Если ПланыСчетов.Хозрасчетный.НайтиПоКоду(Док.СчетЗатрат.Код).ВидыСубконто[0].ВидСубконто.ИмяПредопределенныхДанных = "НоменклатурныеГруппы" тогда 
				Док.Субконто1 = Справочники.НоменклатурныеГруппы.НайтиПоНаименованию("Основная номенклатурная группа");
				Док.Субконто2 = Объект.Статьи.НайтиСТроки(Новый Структура("СтатьяРасходовИИКО", НСтр[0].СтатьяРасходовИИКО))[0].СтатьяЗатрат1С;
			Иначе
				Док.Субконто1 = Объект.Статьи.НайтиСТроки(Новый Структура("СтатьяРасходовИИКО", НСтр[0].СтатьяРасходовИИКО))[0].СтатьяЗатрат1С;
				Док.Субконто2 = Справочники.НоменклатурныеГруппы.НайтиПоНаименованию("Основная номенклатурная группа");
			КонецЕсли;
		Иначе
			Если ПланыСчетов.Хозрасчетный.НайтиПоКоду(Док.СчетЗатрат.Код).ВидыСубконто[0].ВидСубконто.ИмяПредопределенныхДанных = "НоменклатурныеГруппы" тогда 
				Док.Субконто1 = Справочники.НоменклатурныеГруппы.НайтиПоНаименованию("Основная номенклатурная группа");
...
Процедура СоздатьВозвратныеНакладные(табз,префикс,ДеньПрефикс,порядок)
	...
	Для каждого Стр из Доки цикл
		...
		ТабЧасть = Новый ТаблицаЗначений;
		Табчасть = Табз.Скопировать(НСтр);
		Табчасть.Свернуть("НКодИИКО,ННаименованиеИИКО,СтавкаНДСспродаж,СтавкаНДС,ГруппаУчетаНазвание","СуммаПродажисНДС,СуммаНДСспродаж,СебестоимостьбезНДС,Количество,СебестоимостьзаедбезНДС");
		...
		Для каждого стрр из Табчасть Цикл 
			...
			Иначе
				...
				Стр2.Сумма = стрр.СуммапродажисНДС;
				Стр2.СуммаНДС = стрр.СуммаНДСспродаж;
				стр2.Себестоимость = Стр2.Количество;
			КОнецЕсли;
...
Процедура СоздатьИнвентаризации(табз,префикс,порядок)
	...
	Для каждого Стр из Доки цикл
		...
		Для каждого стрр из ТабЧасть Цикл 
			...
			//Стр2.Цена = стрр.СебестоимостьБезНДС/стрр.Количество;
...
Процедура СоздатьПКВ(табз,префикс,порядок, тз1, ТЗ10)	
	...
	Если ПКО_АР тогда 
		...
		Для каждого Стр из Доки2 цикл		
			...
			Отбор = Новый Структура("Дата, ВидДок", НачалоДня(ДатаДокум), "Акт Реализации");  //Новый Структура("Дата",НачалоДня(ДатаДокум));//
...
Процедура СоздатьАвансовыйОтчет(докСтр,строки,НомерДокумента, Порядок, номернаклиико, ВидДок)
	...
	Док.Организация = НайтиОгранизациюПоИНН(докСтр.ЮЛИНН, Ложь);
	Если Метаданные.Версия > "3.0.69.00" тогда 
	...
	Если ВидДок = "Приходная Накладная" тогда 
		Док.Склад = Объект.Склады.НайтиСтроки(Новый Структура ("СкладНаименование",докСтр.Склад))[0].Склад;//докСтр.Склад;
		Док.ПодразделениеОрганизации = Объект.Склады.НайтиСтроки(Новый Структура("Склад",Док.Склад))[0].Подразделение;
	КонецЕсли;
...
Процедура СохранитьКатегории()
	...
	Для каждого стр из Объект.БухгалтерскаяКатегория цикл
		...
		Если ЗначениеЗаполнено(строка) Тогда
			...
			Если ТекущаяНастройка.СохраненнаяНастройка.Категории.Колонки.Найти("ВидНоменклатуры") <> Неопределено тогда
				строка.ВидНоменклатуры = Стр.ВидНоменклатуры;					
			КонецЕсли;
		Иначе
			...
			Если ТекущаяНастройка.СохраненнаяНастройка.Категории.Колонки.Найти("ВидНоменклатуры") <> Неопределено тогда
				новаяСтрока.ВидНоменклатуры = Стр.ВидНоменклатуры;					
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
...
Процедура СоздатьАПУ (табз,префикс,ДеньПрефикс,порядок)
	...
	Для Каждого Стр из табз Цикл
		...
		Попытка
			ххх.Контрагент = Объект.Контрагенты.НайтиСтроки(Новый Структура("КкодИИКО", Стр.КкодИИКО))[0].Контрагент;
		Исключение
			ххх.Контрагент = Неопределено;//Объект.Контрагенты.НайтиСтроки(Новый Структура("КкодИИКО", Стр.КкодИИКО))[0].Контрагент;
		КонецПопытки;
	...
	Доки.Свернуть("Номер,Дата,ТорговоеПредприятиеКод,ЮЛнаименование,ЮЛИНН,Контрагент,СчетФактура,ВходНомер,ВходДата,Сотрудник,Комментарий");
	 ...
	 Для каждого докСтр из Доки цикл
		...
		// Списать округление
		Если ДокСтр.КОнтрагент = Неопределено тогда
				СписатьОкругление(ДокСтр,Тзз1);
				Продолжить;
		КонецЕсли;
...
Процедура КонтрагентыПередУдалением(Элемент, Отказ)
	НаимУдаляемых ="";
	Для каждого Номер из Элементы.Контрагенты.ВыделенныеСтроки  Цикл
		
	НаимУдаляемых = Объект.Контрагенты.НайтиПоИдентификатору(Номер).КНаименованиеИИКО + ", " + НаимУдаляемых;
	
	КонецЦикла;
	
	Если Элементы.Контрагенты.ВыделенныеСтроки.Количество() = 1 Тогда
		ТекстВопроса = "Удалить из загрузки все документы с контрагентом: ";
	Иначе
		ТекстВопроса = "Удалить из загрузки все документы с контрагентами: ";
	КонецЕсли;
	...
	КодАйко= Объект.Контрагенты.НайтиПоИдентификатору(Номер).КНаименованиеИИКО;
...
&НаСервере
Процедура СписатьОкругление(ДокСтр, Тзз1)
	
	СписокДокументов = Объект.Докум.НайтиСтроки(Новый Структура("Дата, ВидДок",НачалоДня(ДокСтр.Дата),"Акт Реализации"));
	НужныеСтроки = Тзз1.НайтиСтроки(Новый Структура("Номер,Дата",ДокСтр.Номер,ДокСтр.Дата));
	НужнаяТаблица = Тзз1.Скопировать(НужныеСтроки);//.Итог("СуммасНДС");
	СуммаОкругления = НужнаяТаблица.Итог("СуммасНДС");
	Для каждого Стр из СписокДокументов цикл
		Если ЗначениеЗаполнено(Стр.Док) И СуммаОкругления < Стр.Док.СуммаДокумента тогда
			ДокООРП = Стр.Док.ПолучитьОбъект();
			СуммаДокумента = ДокООРП.СуммаДокумента;
			Для каждого строка из ДокООРП.Товары цикл
				Строка.Сумма = Строка.Сумма - (Строка.Сумма/ДокООРП.СуммаДокумента) * СуммаОкругления;
				Строка.Цена = Строка.Сумма/Строка.Количество;					
			КонецЦикла;
			
			//Добавим в последнюю строку лишнюю сумму
			Если ДокООРП.Товары.Итог("Сумма")  <> (СуммаДокумента - СуммаОкругления) тогда 
				Разница = ДокООРП.Товары.Итог("Сумма") - (ДокООРП.Товары.Итог("Сумма") - СуммаОкругления);
				ДокООРП.Товары[ДокООРП.Товары.Количество()-1].Сумма = ДокООРП.Товары[ДокООРП.Товары.Количество()-1].Сумма + ?(ДокООРП.Товары.Итог("Сумма") > (ДокООРП.Товары.Итог("Сумма") - СуммаОкругления),- Разница,Разница);//Макс(ДокООРП.Товары.Итог("Сумма") - СуммаВсего, -(ДокООРП.Товары.Итог("Сумма") - СуммаВсего));
				ДокООРП.Товары[ДокООРП.Товары.Количество()-1].Цена = ДокООРП.Товары[ДокООРП.Товары.Количество()-1].Сумма / ДокООРП.Товары[ДокООРП.Товары.Количество()-1].Количество;
			КонецЕсли;
			Если ДокООРП.Проведен тогда
				ДокООРП.Записать(РежимЗаписиДокумента.Проведение);
				Прервать;
			Иначе
				ДокООРП.Записать(РежимЗаписиДокумента.Запись);
				Прервать;
			КонецЕсли;
		КонецЕсли;		
	КонецЦикла;
...
Процедура ПроверитьНастройкиНаМетаданные(ТекущаяНастройка)
	
	    Если ТекущаяНастройка.СохраненнаяНастройка.Категории.Колонки.Найти("ВидНоменклатуры") = Неопределено тогда
			ТекущаяНастройка.СохраненнаяНастройка.Категории.Колонки.Добавить("ВидНоменклатуры")					
		КонецЕсли;
