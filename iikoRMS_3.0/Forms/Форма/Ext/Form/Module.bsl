...
Процедура ЗагрузитьНаСервере()
	...
	Если АС и ЗначениеЗаполнено(АдресАС) Тогда
		...
		НН = Тз6.Количество()-1;
		Пока НН >= 0 Цикл
			Тз6[НН].Количество = УдалитьПробелы(Тз6[НН].Количество);
			Если Число(Тз6[НН].Количество) = 0 Тогда
				Тз6.Удалить(Тз6[НН]);
				НН = НН - 1;
			Иначе
				Тз6[НН].СебестоимостьзаедбезНДС = УдалитьПробелы(Тз6[НН].СебестоимостьзаедбезНДС);
				Тз6[НН].СебестоимостьбезНДС = УдалитьПробелы(Тз6[НН].СебестоимостьбезНДС); 				
				Тз6[НН].СкладНаименование ="("+ Тз6[НН].Торговоепредприятиенаименование + ") " + Тз6[НН].СкладНаименование; 				
				Если НомерПлюсКодТП тогда 
					Тз6[НН].НомерДок = Тз6[НН].ТорговоеПредприятиеКод + Тз6[НН].НомерДок;
				КонецЕсли;
				ЕдИзм = ТаблицаЕдиницыИзмерений.НайтиСтроки(Новый Структура("ЕдиницаИзмеренияАйко",Тз6[НН].Единицаизмеренияназвание));
				Если ЕдИзм.Количество() > 0 тогда 
				Иначе
					НоваяЕдИзм = ТаблицаЕдиницыИзмерений.Добавить();
					НоваяедИзм.ЕдиницаИзмеренияАйко = Тз6[НН].Единицаизмеренияназвание;
				КонецЕсли;
				НН = НН -1;
			КонецЕсли;	
		КонецЦикла;  		
